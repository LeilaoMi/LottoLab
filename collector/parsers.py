...
def parse_cwl(text: str, kind: str):
    ...
    for it in data["result"]:
        ...
        out.append({"issue": issue, "main": main, "special": special, "draw_date": date,
                    "sales": sales, "pool": pool, "prizes": prizes})
    return out
...
def parse_500(text: str, kind: str):
    ...
    for row in _ROW.findall(text):
        ...
        dm = _DATE.search(row); im = _ISSUE.search(row)
        if not (dm and im):
            continue
...
def parse_17500(text: str, kind: str, main_count: int, special_count: int = 0, last_hi: int | None = None):
    ...
    for ln in lines:
        ...
        special = [int(x) for x in p[2 + main_count:2 + main_count + special_count]] if special_count else []
