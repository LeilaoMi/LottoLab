...
from contract import ContractError, canonicalize, key
...
def ingest(kind: str, records_by_source: dict[str, list[dict]], today: str | None = None):
    ...
    for src, recs in records_by_source.items():
        for raw in recs:
            try:
                c = canonicalize(
                    kind, raw["issue"], raw.get("main", []), raw.get("special", []), raw.get("draw_date", "")
                )
    ...
    for issue, srcmap in by_issue.items():
        ...
        if len(set(fp.values())) > 1:
            rejected.setdefault(
                issue, {"reason": ["多源不一致，拒绝落库"], "variants": {s: list(v) for s, v in fp.items()}}
            )
        else:
    ...
    return {
        "accepted": accepted,
        "rejected": rejected,
        "per_source": {s: len(r) for s, r in records_by_source.items()},
    }
...
def freshness(snapshot: dict, today: str, max_age_days: int):
    ...
    for kind, rows in snapshot.items():
        ...
        rep[kind] = {
            "latest_issue": (rows[-1]["issue"] if rows else None),
            "latest_date": latest,
            "age_days": age,
            "ok": age <= max_age_days,
        }
